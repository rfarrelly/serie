...
def test_edge_calculator():
    ...
    try:
        ...
        # Sharp odds (fair market) - adjusted to have typical positive overround
        sharp_odds = MarketOdds(
            home=Odds(Decimal("2.10")),  # Slightly lower odds = higher overround
            draw=Odds(Decimal("3.20")),
            away=Odds(Decimal("3.80")),
        ...
        print(f"   Home EV: {edge_result.expected_values[BetType.HOME]:.4f}")
        ...
        assert no_vig_result.overround > 0, (
            "Overround should be positive (bookmaker profit margin)"
        )
